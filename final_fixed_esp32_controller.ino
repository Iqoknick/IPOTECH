...
void loop() {
  ...
  if(millis() - lastFirebaseRead > firebaseInterval){
    ...
    // 2. Heater Control Logic (Manual + Hysteresis - No Override)
    ...
    if(heaterStopAt > 0 && millis() >= heaterStopAt && ovenHeaterState){
      ...
      Firebase.setInt(fb_do, pathHeaterStopAt, 0); // Reset timer
    }
    
    // App control for heater (always works like pulverizer)
    ...
    // Hysteresis control (works alongside app control)
    if(tempC <= tempLow && !ovenHeaterState){
      ovenHeaterState = true;
      digitalWrite(SSR_OVEN, HIGH);
      Firebase.setBool(fb_do, pathHeaterRelay, true);
    }
    else if(tempC >= tempHigh && ovenHeaterState){
      ovenHeaterState = false;
      digitalWrite(SSR_OVEN, LOW);
      Firebase.setBool(fb_do, pathHeaterRelay, false);
    }
    ...
    if(conveyorStopAt > 0 && millis() >= conveyorStopAt && conveyorState && !conveyorManualOverride){
      ...
      Firebase.setInt(fb_do, pathConveyorStopAt, 0); // Reset timer
    }
  ...
  // HEATER BUTTON (GPIO 13): Direct manual control (like pulverizer)
